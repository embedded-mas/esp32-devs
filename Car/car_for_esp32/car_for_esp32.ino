...
const int TRIG_FRENTE = 32;
const int ECHO_FRENTE = 35;

const int LIGHT_ALERT = 26;
const int LIGHT_CONTROL = 34;
...
/*
 FUNÇÃO PARA O ESP32 QUE FAZ A LEITURA DA DISTANCIA ATUAL CALCULADA PELO SENSOR
*/
int getDistance()
{
 ...
 distanciaCM = sensorFrente.convert(microsec, Ultrasonic::CM);
...
void loop()
{
  ...
  //---------------------------------------------------------  PROCESSAMENTO  -------------------------------------------------------
  distanciaFrente = getDistance();// ultrassom.Ranging(CM) retorna a distancia em centímetros(CM) ou polegadas(INC)
